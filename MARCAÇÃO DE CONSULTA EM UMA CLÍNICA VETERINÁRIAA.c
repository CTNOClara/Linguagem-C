...
//função para criar arvore/inicicalizar arvore
TNO* criarArvore(){ 
TNO* Arvore = (TNO*) malloc(sizeof(TNO));
...
//funções que serão tratadas para o processo de inserção
TNO *inserirNo(TNO *pai, TNO *no, int indiceice) {
    pai -> filho[indiceice] = no;
...
//funções que serão tratadas para a remoção de elementos da arvore
int buscar(TNO *no) {
    int aux = no -> chave[0];
...
int main() {
  int num;
    ...
    TPilha* Pilha = criarPilha();
     printf("*CLINICA VETERINÁRIA*\n");
	printf("Bem vindo ao Sistema de Agendamento de Consultas do seu PET<3\n-Obs:As consultas deverão ser marcadas com ate 24 horas de antecedência!\n");
	printf("-As consultas vão de 8h até às 19h. E os atendimentos vão de 2a a sábado.\n\n");
	...
	switch(codigo){
		case 1:
			printf("Diga qual dia e horário você deseja marcar a consulta:");
			scanf("%d",&dataehora);
			printf("Qual o nome do seu PET?:");
			scanf("%s",&nomedopet);
			//verfica se ta disponivel, se não estiver insere na arvore
			Arv = inserir(Arv, dataehora, Pilha); 
			//se não estiver disponivel escolha uma nova data
			printf("\nO agendamento do(a) %s foi concluido com sucesso.",nomedopet);
		break;
		
		case 2:
			printf("Para qual dia a consulta estaria marcada?");
			//salva a informação e faz a remoção e balancemento da arvore.
			printf("Diga qual dia e horário você deseja remarcar a consulta:");
			scanf("%d",&dataehora);
			//verfica se ta disponivel, se não estiver insere na arvore
			Arv = inserir(Arv, dataehora, Pilha); 
			//se não estiver disponivel escolha uma nova data
			printf("Seu agendamento foi concluido com sucesso.");
		break;
		...
		case 3:
			...
			//salva a informação e faz a remoção e balancemento da arvore.
			scanf("%d",&dataehora);
			//fazer remoção da chave
			printf("Sua consulta foi cancelada");
		break;
  }
}
